...
const engine = ENGINE_CONFIGS.find(
  (candidate) =>
...
if (engine) {
  initialize().catch((error) => {
    console.error(
      "SERP Domain Index v2 failed to initialize:",
      error
    );
  });
}

function anchorFromPreferredMatch(
  result,
  selector
) {
  const match =
...
function savePreviousDisplay(result) {
  if (
    result.dataset
      .serpDomainIndexHidden === "1"
  ) {
    return;
  }

  result.dataset
    .serpDomainIndexPreviousDisplay =
    result.style.getPropertyValue(
      "display"
    );

  result.dataset
    .serpDomainIndexPreviousPriority =
    result.style.getPropertyPriority(
      "display"
    );
}

function restorePreviousDisplay(result) {
...
async function initialize() {
  ...
  function requestProcessing() {
    ...
    requestAnimationFrame(() => {
      ...
      processPendingResults().catch(
        (error) => {
          console.error(
            "SERP Domain Index v2 processing failed:",
            error
          );
        }
      );
  ...
  async function performLookup(
    ...
    for (
      ...
      try {
        ...
        console.error(
          "SERP Domain Index v2 lookup failed:",
          error
        );
